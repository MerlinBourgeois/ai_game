...
# Paramètres pour la sauvegarde et la libération de mémoire
checkpoint_path = "model_checkpoint.ckpt"
memory_clear_interval = 1000  # Intervalles d'itération avant de libérer la mémoire
save_interval = 1000  # Intervalles d'itération avant de sauvegarder le modèle
...
while running:
    ...
    for _ in range(iterations):
        ...
        # Recharger le modèle mis à jour
        if iteration_count % save_interval == 0:
            checkpoint.restore(tf.train.latest_checkpoint(checkpoint_path))
            print("Modèle rechargé pour amélioration à l'itération", iteration_count)
